...
function getGoodsCopy(good: GoodsRecord, stock: number) {
  const rawDescription =
    typeof good.description === 'string' ? good.description.trim() : ''
  const fallbackExtra = Number.isFinite(stock) ? `剩余 ${stock} 件` : ''

  if (!rawDescription) {
    return {
      priceSuffix: '',
      extraText: fallbackExtra
    }
  }

  const parts = rawDescription.split('||').map(part => part.trim())
  if (parts.length >= 2) {
    return {
      priceSuffix: parts[0],
      extraText: parts[1] || fallbackExtra
    }
  }

  return {
    priceSuffix: '',
    extraText: rawDescription
  }
}
...
export function attachLampRuntime() {
  ...
  function buildSlot(good: GoodsRecord) {
    ...
    const { priceSuffix, extraText } = getGoodsCopy(good, stock)
    priceEl.textContent = `¥ ${formatPriceAmount(safeNum(good.price))}${priceSuffix}`
